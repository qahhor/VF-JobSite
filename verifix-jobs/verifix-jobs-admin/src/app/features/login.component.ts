    ...
    <div class="min-h-screen bg-slate-900 flex items-center justify-center p-4">
      <div class="w-full max-w-sm">
        <div class="text-center mb-8">
          ...
          <h1 class="text-xl font-bold text-white">{{ i18n.t('admin.brand') }}</h1>
          <p class="text-slate-400 text-sm mt-1">{{ i18n.t('admin.panel') }}</p>
        </div>
        <div class="bg-white rounded-2xl p-6 shadow-2xl">
          ...
          <form (ngSubmit)="onSubmit()" class="space-y-4">
            <div>
              <label class="block text-xs font-medium text-gray-600 mb-1">{{ i18n.t('admin.email') }}</label>
            ...
            <div>
              <label class="block text-xs font-medium text-gray-600 mb-1">{{ i18n.t('admin.password') }}</label>
            ...
            <div>
              <label class="block text-xs font-medium text-gray-600 mb-1">{{ i18n.t('admin.totp') }}</label>
            ...
            <button type="submit" [disabled]="loading()" class="w-full py-2.5 bg-black text-white rounded-lg text-sm font-medium hover:bg-gray-800 transition disabled:opacity-50">
              {{ loading() ? i18n.t('admin.logging_in') : i18n.t('admin.login') }}
            </button>
          </form>
        </div>
      </div>
...
export class AdminLoginComponent {
  ...
  constructor(private api: AdminApiService, private router: Router, public i18n: I18nService) {}
