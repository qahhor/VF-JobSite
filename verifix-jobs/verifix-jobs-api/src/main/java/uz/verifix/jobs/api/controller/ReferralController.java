...
public class ReferralController {
    ...
    public ResponseEntity<List<ReferralResponse>> getMyReferrals(@PathVariable UUID candidateId) {
        List<Referral> referrals = referralService.getByReferrer(candidateId);
        return ResponseEntity.ok(referrals.stream().map(this::toResponse).toList());
    }

    @GetMapping("/my/{candidateId}/stats")
    public ResponseEntity<ReferralService.ReferralStats> getMyStats(@PathVariable UUID candidateId) {
        return ResponseEntity.ok(referralService.getStats(candidateId));
    }
