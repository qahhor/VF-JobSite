      ...
      <form (submit)="handleSubmit($event)" class="space-y-6">
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          @if (step() === 0) {
            <div class="space-y-4">
              <div>
                ...
                <input type="text" [(ngModel)]="form.title" [ngModelOptions]="{ standalone: true }" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-black/20 focus:border-primary outline-none" placeholder="Masalan: Bosh oshpaz" aria-label="Vakansiya nomi" aria-required="true">
              </div>
          ...
          @if (step() === 1) {
            <div class="space-y-4">
              <div>
                ...
                <textarea [(ngModel)]="form.description" [ngModelOptions]="{ standalone: true }" rows="6" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-black/20 focus:border-primary outline-none" placeholder="Ish haqida batafsil..."></textarea>
              </div>
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
          ...
          @if (step() === 2) {
            <div class="space-y-4">
              <div>
                ...
                <input type="text" [(ngModel)]="benefitsStr" [ngModelOptions]="{ standalone: true }" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm" placeholder="Ovqat, transport, uy-joy...">
              </div>
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
          ...
          <div class="flex justify-between mt-6 pt-4 border-t border-gray-100">
            ...
            @if (step() < 4) {
              <button type="button" (click)="nextStep()" class="px-4 py-2 bg-black text-white rounded-lg text-sm hover:bg-gray-800">Keyingi</button>
            } @else {
              <button type="submit" [disabled]="saving()" class="px-6 py-2 bg-secondary text-white rounded-lg text-sm font-medium hover:bg-secondary-600 disabled:opacity-50">
...
export class VacancyFormComponent implements OnInit {
  ...
  submit() {
    if (!this.form.title?.trim()) {
      this.formError.set('Sarlavha kiritilmagan');
      return;
    }
    ...
    req.subscribe({
      ...
      error: (err: any) => {
        ...
        this.formError.set(err.error?.message || 'Xatolik yuz berdi');
      },
    });
  }
}
