    ...
    <div class="space-y-5">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-slate-500">{{ i18n.t('admin.experiments.hint') }}</p>
        </div>
        <button (click)="showCreate.set(true)" class="shrink-0 rounded-2xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800">
      ...
      @if (loading()) {
        ...
        <div class="rounded-[28px] border border-slate-200 bg-white p-12 text-center">
          ...
          <div class="mt-3 text-sm text-slate-500">{{ i18n.t('admin.experiments.empty') }}</div>
        </div>
      } @else {
            ...
            <div class="rounded-[20px] border border-slate-200 bg-white p-4 transition hover:border-slate-300">
              <div class="flex items-start justify-between gap-2">
                <div class="min-w-0">
                  ...
                  @if (exp.description) {
                    <div class="mt-0.5 text-xs text-slate-500">{{ exp.description }}</div>
                  }
                </div>
                <span class="shrink-0 rounded-xl px-2.5 py-1 text-xs font-medium"
                  [class]="exp.active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'">
              ...
              <div class="mt-3 flex flex-wrap gap-2">
                @if (exp.active) {
                  <button (click)="toggleActive(exp, false)" class="rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 transition hover:bg-slate-50">
                ...
                <button (click)="viewStats(exp)" class="rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 transition hover:bg-slate-50">
              ...
              @if (selectedStats()?.name === exp.name) {
                <div class="mt-3 rounded-xl border border-slate-100 bg-slate-50 p-3">
                  @if (selectedStats()?.winner === 'INSUFFICIENT_DATA') {
                    <div class="text-xs text-slate-500">{{ i18n.t('admin.experiments.not_enough_data') }}</div>
                  } @else {
                    ...
                    @if (selectedStats()?.winner && selectedStats()?.winner !== 'TIE') {
                      ...
                      <div class="mt-2 text-xs text-slate-500">{{ i18n.t('admin.experiments.tie') }}</div>
                    }
                  }
                </div>
              }
      ...
      <!-- Create modal -->
      @if (showCreate()) {
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/20" (click)="showCreate.set(false)">
          <div class="w-full max-w-md rounded-[28px] border border-slate-200 bg-white p-6 shadow-lg" (click)="$event.stopPropagation()">
            ...
            <div class="mt-4 space-y-3">
              <input
                ...
                class="h-11 w-full rounded-2xl border border-slate-300 px-4 text-sm outline-none focus:border-slate-950" />
              <input
                [(ngModel)]="newDesc"
            ...
            <div class="mt-5 flex justify-end gap-2">
              <button (click)="showCreate.set(false)" class="rounded-2xl border border-slate-200 px-4 py-2.5 text-sm transition hover:bg-slate-50">
                {{ i18n.t('admin.cancel') }}
              </button>
              <button
                (click)="create()"
                [disabled]="!newName.trim()"
                class="rounded-2xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:opacity-50">
