    ...
    <div class="space-y-5">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-slate-500">{{ i18n.t('admin.audit.hint') }}</p>
        </div>
        <button
          (click)="exportCsv()"
          class="shrink-0 rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:border-slate-300 hover:bg-slate-50">
      ...
      @if (loading()) {
        ...
        <div class="rounded-[28px] border border-slate-200 bg-white p-12 text-center">
          ...
          <div class="mt-3 text-sm text-slate-500">{{ i18n.t('admin.audit.empty') }}</div>
        </div>
      } @else {
        <div class="space-y-2">
          @for (log of logs(); track log.id) {
            <div
              class="rounded-[20px] border border-slate-200 bg-white transition hover:border-slate-300"
              [class.border-slate-300]="expandedId() === log.id">
              <button
                (click)="toggle(log)"
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex flex-wrap items-center gap-2">
                    ...
                    <span class="text-xs text-slate-500">{{ log.adminEmail }}</span>
                  </div>
                ...
                <svg class="h-4 w-4 shrink-0 text-slate-400 transition" [class.rotate-180]="expandedId() === log.id" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              @if (expandedId() === log.id && log.details) {
                <div class="border-t border-slate-100 px-4 py-3">
                  <pre class="max-h-40 overflow-auto rounded-xl bg-slate-50 p-3 text-xs text-slate-600">{{ formatDetails(log.details) }}</pre>
                </div>
              }
            </div>
          }
        ...
        @if (totalPages() > 1) {
          <div class="flex items-center justify-center gap-3 pt-2">
            <button [disabled]="page() === 0" (click)="goPage(page() - 1)" class="rounded-xl border border-slate-200 px-4 py-2 text-sm transition hover:bg-slate-50 disabled:opacity-40">&larr;</button>
            <span class="text-sm text-slate-600">{{ page() + 1 }} / {{ totalPages() }}</span>
            <button [disabled]="page() >= totalPages() - 1" (click)="goPage(page() + 1)" class="rounded-xl border border-slate-200 px-4 py-2 text-sm transition hover:bg-slate-50 disabled:opacity-40">&rarr;</button>
          </div>
        }
      }
    </div>
