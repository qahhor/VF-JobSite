    ...
    <div class="space-y-6">
      <section class="rounded-2xl border border-border bg-white p-6 shadow-card">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div class="text-sm uppercase tracking-[0.24em] text-primary">{{ i18n.t('admin.access') }}</div>
          ...
          <div class="grid gap-3 sm:grid-cols-3">
            <div class="rounded-2xl border border-border bg-surface p-4">
              <div class="text-xs uppercase tracking-[0.18em] text-muted">{{ i18n.t('admin.my_role') }}</div>
              <div class="mt-2 text-xl font-semibold">{{ roleLabel(profile()?.role) }}</div>
            </div>
            <div class="rounded-2xl border border-border bg-surface p-4">
              <div class="text-xs uppercase tracking-[0.18em] text-muted">{{ i18n.t('admin.2fa.title') }}</div>
      ...
      <div class="grid gap-6 xl:grid-cols-[1.15fr_1fr]">
        <section class="space-y-6">
          <div class="rounded-2xl border border-border bg-white p-6 shadow-card">
            ...
            <p class="mt-2 text-sm text-muted">{{ i18n.t('admin.profile_security_hint') }}</p>
            <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <div class="rounded-2xl border border-border bg-surface p-4">
                <div class="text-xs uppercase tracking-[0.18em] text-muted">{{ i18n.t('admin.email') }}</div>
                <div class="mt-2 text-sm font-medium">{{ profile()?.email || 'admin@verifix.uz' }}</div>
              </div>
              <div class="rounded-2xl border border-border bg-surface p-4">
                <div class="text-xs uppercase tracking-[0.18em] text-muted">{{ i18n.t('admin.created_at') }}</div>
                <div class="mt-2 text-sm font-medium">{{ profile()?.createdAt ? (profile()?.createdAt | date:'dd.MM.yyyy HH:mm') : '-' }}</div>
              </div>
              <div class="rounded-2xl border border-border bg-surface p-4">
                <div class="text-xs uppercase tracking-[0.18em] text-muted">{{ i18n.t('admin.last_login') }}</div>
                <div class="mt-2 text-sm font-medium">{{ profile()?.lastLoginAt ? (profile()?.lastLoginAt | date:'dd.MM.yyyy HH:mm') : '-' }}</div>
              </div>
              <div class="rounded-2xl border border-border bg-surface p-4">
                <div class="text-xs uppercase tracking-[0.18em] text-muted">{{ i18n.t('admin.last_password_change') }}</div>
            ...
            <div class="rounded-2xl border border-border bg-white p-6 shadow-card">
              <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p class="mt-2 text-sm text-muted">{{ i18n.t('admin.2fa.hint') }}</p>
                </div>
                <button
                  (click)="setupTwoFactor()"
                  [disabled]="settingUpTwoFactor()"
                  class="rounded-2xl bg-primary px-4 py-3 text-sm font-semibold text-white transition hover:bg-primary-600 disabled:cursor-not-allowed disabled:opacity-60">
              ...
              @if (twoFactorSetup()) {
                <div class="mt-4 rounded-2xl border border-border bg-surface p-4">
                  ...
                  <div class="mt-2 break-all rounded-xl bg-white px-3 py-3 font-mono text-sm border border-border">{{ twoFactorSetup()?.secret }}</div>
                  <div class="mt-4 flex flex-wrap gap-3">
                    <button
                      type="button"
                      (click)="copyValue(twoFactorSetup()?.secret || '')"
                      class="rounded-2xl border border-border px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-border hover:bg-slate-100">
                      {{ i18n.t('admin.copy_secret') }}
                    </button>
                    <a
                      class="rounded-2xl bg-primary px-4 py-2 text-center text-sm font-semibold text-white transition hover:bg-primary-600"
            ...
            <form (ngSubmit)="changePassword()" class="mt-5 space-y-4">
              <div class="grid gap-4 md:grid-cols-2">
                <label class="block">
                  ...
                  <input [(ngModel)]="passwordForm.currentPassword" name="currentPassword" type="password" autocomplete="current-password" class="h-12 w-full rounded-2xl border border-border bg-white px-4 text-sm outline-none transition focus:border-slate-950" required />
                </label>
                <label class="block">
                  <span class="mb-2 block text-sm font-medium text-slate-700">{{ i18n.t('admin.new_password') }}</span>
                  <input [(ngModel)]="passwordForm.newPassword" name="newPassword" type="password" autocomplete="new-password" class="h-12 w-full rounded-2xl border border-border bg-white px-4 text-sm outline-none transition focus:border-slate-950" required />
                </label>
              </div>
              <div class="flex flex-col gap-3 rounded-2xl border border-border bg-surface p-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="text-xs leading-5 text-muted">{{ i18n.t('admin.password_policy') }}</div>
                <button type="button" (click)="fillGeneratedNewPassword()" class="rounded-2xl border border-border px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-border hover:bg-slate-100">
              ...
              <label class="block">
                ...
                <input [(ngModel)]="passwordForm.confirmPassword" name="confirmPassword" type="password" autocomplete="new-password" class="h-12 w-full rounded-2xl border border-border bg-white px-4 text-sm outline-none transition focus:border-slate-950" required />
              </label>
              @if (passwordError()) {
                <div class="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{{ passwordError() }}</div>
              }
              <button type="submit" [disabled]="savingPassword()" class="inline-flex rounded-2xl bg-primary px-5 py-3 text-sm font-semibold text-white transition hover:bg-primary-600 disabled:cursor-not-allowed disabled:opacity-60">
          ...
          @if (issuedCredential()) {
            <div class="rounded-2xl border border-emerald-200 bg-emerald-50 p-6">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="rounded-full px-3 py-1 text-xs font-medium" [class]="issuedCredential()?.emailSent ? 'border border-border bg-white text-slate-700' : 'border border-amber-200 bg-amber-100 text-amber-800'">
              ...
              <div class="mt-5 grid gap-4 sm:grid-cols-2">
                <div class="rounded-2xl border border-emerald-200 bg-white p-4">
                  ...
                  <button type="button" (click)="copyValue(issuedCredential()?.email || '')" class="mt-3 rounded-2xl border border-border px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-border hover:bg-slate-100">
                    {{ i18n.t('admin.copy_email') }}
                  </button>
                </div>
                <div class="rounded-2xl border border-emerald-200 bg-white p-4">
                  <div class="text-xs uppercase tracking-[0.18em] text-muted">{{ i18n.t('admin.temporary_password') }}</div>
                  <div class="mt-2 break-all font-mono text-sm font-medium">{{ issuedCredential()?.password }}</div>
                  <button type="button" (click)="copyValue(issuedCredential()?.password || '')" class="mt-3 rounded-2xl border border-border px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-border hover:bg-slate-100">
            ...
            <div class="rounded-2xl border border-border bg-white p-6 shadow-card">
              <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  ...
                  <p class="mt-2 text-sm text-muted">{{ i18n.t('admin.team_access_hint') }}</p>
                </div>
                <div class="w-full sm:w-72">
                  <label class="mb-2 block text-sm font-medium text-slate-700">{{ i18n.t('common.search') }}</label>
                  <input [(ngModel)]="adminSearch" (ngModelChange)="loadAdmins()" type="text" class="h-12 w-full rounded-2xl border border-border bg-white px-4 text-sm outline-none transition focus:border-slate-950" [placeholder]="i18n.t('admin.search_admins')" />
                </div>
              </div>
                  ...
                  <div class="rounded-2xl border border-border bg-surface p-4">
                    <div class="flex flex-wrap items-center gap-2">
                      ...
                      @if (admin.currentUser) {
                        <span class="rounded-full border border-border bg-white px-2.5 py-1 text-[11px] font-medium text-slate-700">{{ i18n.t('admin.current_session') }}</span>
                      }
                    ...
                    @if (admin.inviteSentAt) {
                      <div class="mt-2 text-xs text-muted">{{ i18n.t('admin.invite_sent_at') }}: {{ admin.inviteSentAt | date:'dd.MM.yyyy HH:mm' }}</div>
                    }
                    <div class="mt-4 grid gap-3 sm:grid-cols-[1fr_auto]">
                      <select [(ngModel)]="roleDrafts[admin.id]" [ngModelOptions]="{ standalone: true }" [disabled]="!canManageAdmins() || admin.currentUser" class="h-11 rounded-2xl border border-border bg-white px-4 text-sm outline-none transition focus:border-slate-950 disabled:cursor-not-allowed disabled:opacity-50">
                        @for (role of roleOptions; track role) {
                          <option [value]="role">{{ roleLabel(role) }}</option>
                        }
                      </select>
                      <button (click)="updateRole(admin)" [disabled]="!canManageAdmins() || admin.currentUser || roleDrafts[admin.id] === admin.role" class="rounded-2xl border border-border px-4 text-sm font-medium transition hover:border-border hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50">
                        {{ i18n.t('admin.update_role') }}
                      </button>
                    </div>
                    <div class="mt-3 grid gap-3 sm:grid-cols-[1fr_auto_auto]">
                      <input [(ngModel)]="resetPasswords[admin.id]" [ngModelOptions]="{ standalone: true }" type="password" [disabled]="!canManageAdmins()" class="h-11 rounded-2xl border border-border bg-white px-4 text-sm outline-none transition focus:border-slate-950 disabled:cursor-not-allowed disabled:opacity-50" [placeholder]="i18n.t('admin.temporary_password')" />
                      <button type="button" (click)="fillGeneratedResetPassword(admin.id)" [disabled]="!canManageAdmins()" class="rounded-2xl border border-border px-4 text-sm font-medium transition hover:border-border hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50">
                        {{ i18n.t('admin.generate_password') }}
                      </button>
                      <button (click)="resetPassword(admin)" [disabled]="!canManageAdmins() || !resetPasswords[admin.id]" class="rounded-2xl border border-border px-4 text-sm font-medium transition hover:border-border hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50">
                        {{ i18n.t('admin.reset_password') }}
                      </button>
                    </div>
                  </div>
                } @empty {
                  <div class="rounded-2xl border border-dashed border-border bg-surface px-4 py-8 text-center text-sm text-muted">{{ i18n.t('admin.team_empty') }}</div>
                }
              </div>
            </div>
            ...
            <div class="rounded-2xl border border-border bg-white p-6 shadow-card">
              <div class="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p class="mt-2 text-sm text-muted">{{ i18n.t('admin.invite_moderator_hint') }}</p>
                </div>
                @if (!canManageAdmins()) {
              ...
              <form (ngSubmit)="inviteAdmin()" class="mt-5 space-y-4">
                <label class="block">
                  ...
                  <input [(ngModel)]="inviteForm.email" name="adminEmail" type="email" class="h-12 w-full rounded-2xl border border-border bg-white px-4 text-sm outline-none transition focus:border-slate-950" [disabled]="!canManageAdmins()" required />
                </label>
                <label class="block">
                  <span class="mb-2 block text-sm font-medium text-slate-700">{{ i18n.t('common.role') }}</span>
                  <select [(ngModel)]="inviteForm.role" name="adminRole" class="h-12 w-full rounded-2xl border border-border bg-white px-4 text-sm outline-none transition focus:border-slate-950" [disabled]="!canManageAdmins()">
                    @for (role of roleOptions; track role) {
                      <option [value]="role">{{ roleLabel(role) }}</option>
                    }
                  </select>
                </label>
                <div class="rounded-2xl border border-border bg-surface p-4 text-xs leading-5 text-muted">{{ i18n.t('admin.invite_email_fallback_hint') }}</div>
                <button type="submit" [disabled]="creatingAdmin() || !canManageAdmins()" class="inline-flex rounded-2xl bg-primary px-5 py-3 text-sm font-semibold text-white transition hover:bg-primary-600 disabled:cursor-not-allowed disabled:opacity-60">
