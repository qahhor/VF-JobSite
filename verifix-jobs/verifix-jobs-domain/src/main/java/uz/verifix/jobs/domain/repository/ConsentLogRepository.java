...
public interface ConsentLogRepository extends JpaRepository<ConsentLog, UUID> {
    ...
    List<ConsentLog> findByUserTypeAndUserIdAndWithdrawnAtIsNull(UserType userType, UUID userId);

    Optional<ConsentLog> findByUserTypeAndUserIdAndConsentTypeAndWithdrawnAtIsNull(
            UserType userType, UUID userId, ConsentType consentType);
}
